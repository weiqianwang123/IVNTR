...
do
    ...
    if python3 predicators/main.py --env blocks_onclear --approach ivntr-pdlm \
        ...
        --approach_dir "saved_approaches/final/blocks_pos/ivntr_pdlm_$seed" \
        --neupi_load_pretrained "saved_approaches/final/blocks_pos/ivntr_pdlm_$seed" \
        --log_file logs/blocks_pos/ivntr_ood_pdlm_test_$seed.log; then
    ...
    if python3 predicators/main.py --env blocks_onclear --approach ivntr-pdlm \
        ...
        --timeout 5 \
        --approach_dir "saved_approaches/final/blocks_pos/ivntr_pdlm_$seed" \
        --log_file logs/blocks_pos/ivntr_indomain_pdlm_test_$seed.log; then
        echo "Seed $seed completed successfully."
    else
        echo "Seed $seed encountered an error."
    fi
