...
do
    ...
    if python3 predicators/main.py --env blocks_onclear --approach ivntr-nosem \
        ...
        --approach_dir "saved_approaches/final/blocks_pos/ivntr_nosem_$seed" \
        --neupi_load_pretrained "saved_approaches/final/blocks_pos/ivntr_nosem_$seed" \
        --log_file logs/blocks_pos/ivntr_ood_nosem_test_$seed.log; then
    ...
    start_time=$(date +%s)
    # low-level sampling is very hard for this environment
    if python3 predicators/main.py --env blocks_onclear --approach ivntr-pdlm \
        --seed $seed --offline_data_method "demo" \
        --disable_harmlessness_check True \
        --excluded_predicates "On,OnTable,GripperOpen,Holding,Clear" \
        --neupi_pred_config "predicators/config/blocks_onclear/pred.yaml" \
        ...
        --timeout 5 \
        --approach_dir "saved_approaches/final/blocks_pos/ivntr_nosem_$seed" \
        --log_file logs/blocks_pos/ivntr_indomain_nosem_test_$seed.log; then
        echo "Seed $seed completed successfully."
    else
        echo "Seed $seed encountered an error."
    fi
