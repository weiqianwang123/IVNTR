...
class LLMEffectVectorGenerator:
    ...
    def _build_system_prompt(self) -> str:
        ...
        lines.append(
            ...
            "The name of the <TARGET> predicate may be unknown—just ignore its name,and using its type and the imcomplete domain PDDL(thinking about what predicates are missing) to infer. "
        )
        lines.append(
        ...
        #PDDL
        lines.append("=== Domain Incomplete PDDL Skeleton ===")
        ...
        lines.append(self._domain_skeleton)
        ...
        lines.append("=== Target Predicate ===")
        lines.append(f"Predicate: Unknown | Types: {[t.name for t in self.target_pred.types]}")
        ...
        #     lines.append(f"{prefix}Predicate: Unknown | Types: {[t.name for t in p.types]}")
         # ----- actions -----
        lines.append("=== Target Predicate Candidate Actions ===")
